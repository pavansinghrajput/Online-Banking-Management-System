...
void balanceEnq(int,struct database);
void jointbalanceEnq(int,struct jointdatabase);
void viewDetails(int, struct database);
void logentry(int account,char c, int amount,int balance)
{
 // moving ofset to the end;
struct trans t;
int flog=open("log.dat",O_RDWR);
lseek(flog,0,SEEK_END);
	time_t curtime;
	time(&curtime);	
 	strcpy(t.time,ctime(&curtime));
    t.account= account;// pass the account number  
    t.amount=amount;// amount that needs to be credit and debit 
    t.action=c; // make 2 if defbit 
	t.famount=balance;
	write(flog,&t,sizeof(t)); // writing the log
	close(flog);
}
...
void viewDetails(int sd, struct database db)
{
	...
	int fd1=open("temp.dat",O_TRUNC|O_RDWR,0666);

	lseek(fd, 0, SEEK_SET);
	lseek(fd1, 0, SEEK_SET);

		
		while(read(fd,&t,sizeof(t))!=0) // taking data in structure format 
		{
			if(t.account==db.acc_id)
				{
				write(fd1,&t,sizeof(struct trans));	
				}
		}
	displayMenu(sd,db);
}
...
void balanceEnq(int sd,struct database db){
...
void jointbalanceEnq(int sd,struct jointdatabase db){
...
{
//i am creating 1 more file accounts2.dat and i will copy whole data from accounts.dat except the record which i want to delete. and then i will copy acccounts2.dat back to account.dat after getting pointer to start.
	char uname[20];
	int fd1, fd2;
