...
void viewDetails(int sd)
{
 ...
 //    struct trans t1;
   ...
   sleep(2);
	...
	int fd=open("temp.dat",O_RDWR);
	if(fd<0)
	{
		printf("file Not read\n");
		exit(0);
	}
	struct trans t; // making a dummy structure to holf the data 
	int i=0;
	while(read(fd,&t,sizeof(t))!=0) // taking data in structure format 
		{
			i=1;
					if(t.action=='D')
					{
						printf("Credit of amount -> %d  at ",t.amount);
					}
					else
						printf("Debit of amount -> %d  at ",t.amount);

					printf("Date time-> %s  in account-> %d \n \n",t.time,t.account);
				
		}
		if(i==0)
			printf("No account present\n");
